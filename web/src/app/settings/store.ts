...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    image_poll_timeout_secs: Number(config.image_poll_timeout_secs || 100),
    image_default_model_slug: String(config.image_default_model_slug || "gpt-5-5-thinking"),
    image_fallback_model_slug: String(config.image_fallback_model_slug || "gpt-5-3"),
    image_account_concurrency: Number(config.image_account_concurrency || 3),
    image_redundant_generation_enabled: Boolean(config.image_redundant_generation_enabled !== false),
    image_redundant_copies: Number(config.image_redundant_copies || 2),
    image_redundant_max_attempts: Number(config.image_redundant_max_attempts || 3),
    ...
    image_chat_panel_enabled: Boolean(config.image_chat_panel_enabled !== false),
...
type SettingsStore = {
  ...
  setImageDefaultModelSlug: (value: string) => void;
  setImageFallbackModelSlug: (value: string) => void;
  setImageAccountConcurrency: (value: string) => void;
  setImageRedundantGenerationEnabled: (value: boolean) => void;
  setImageRedundantCopies: (value: string) => void;
  setImageRedundantMaxAttempts: (value: string) => void;
  setImageSettleEnabled: (value: boolean) => void;
  setImageCheckBeforeHitEnabled: (value: boolean) => void;
  setImageChatPanelEnabled: (value: boolean) => void;
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  saveConfig: async () => {
    ...
    try {
      const data = await updateSettingsConfig({
        ...
        image_poll_timeout_secs: Math.max(1, Number(config.image_poll_timeout_secs) || 100),
        image_default_model_slug: String(config.image_default_model_slug || "gpt-5-5-thinking").trim() || "gpt-5-5-thinking",
        image_fallback_model_slug: String(config.image_fallback_model_slug || "gpt-5-3").trim() || "gpt-5-3",
        image_account_concurrency: Math.max(1, Number(config.image_account_concurrency) || 3),
        image_redundant_generation_enabled: Boolean(config.image_redundant_generation_enabled !== false),
        image_redundant_copies: Math.min(4, Math.max(1, Number(config.image_redundant_copies) || 2)),
        image_redundant_max_attempts: Math.min(5, Math.max(1, Number(config.image_redundant_max_attempts) || 3)),
        image_settle_enabled: Boolean(config.image_settle_enabled !== false),
        image_check_before_hit_enabled: Boolean(config.image_check_before_hit_enabled !== false),
        image_chat_panel_enabled: Boolean(config.image_chat_panel_enabled !== false),
  ...
  setImageDefaultModelSlug: (value) => {
    set((state) => state.config ? { config: { ...state.config, image_default_model_slug: value } } : {});
  },

  setImageFallbackModelSlug: (value) => {
    set((state) => state.config ? { config: { ...state.config, image_fallback_model_slug: value } } : {});
  },
  ...
  setImageRedundantGenerationEnabled: (value) => {
    set((state) => state.config ? { config: { ...state.config, image_redundant_generation_enabled: value } } : {});
  },

  setImageRedundantCopies: (value) => {
    set((state) => state.config ? { config: { ...state.config, image_redundant_copies: value } } : {});
  },

  setImageRedundantMaxAttempts: (value) => {
    set((state) => state.config ? { config: { ...state.config, image_redundant_max_attempts: value } } : {});
  },
  ...
  setImageChatPanelEnabled: (value) => {
    set((state) => state.config ? { config: { ...state.config, image_chat_panel_enabled: value } } : {});
  },
