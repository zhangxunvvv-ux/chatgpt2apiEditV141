...
export function ConfigCard() {
  ...
  const setImageDefaultModelSlug = useSettingsStore((state) => state.setImageDefaultModelSlug);
  const setImageFallbackModelSlug = useSettingsStore((state) => state.setImageFallbackModelSlug);
  const setImageAccountConcurrency = useSettingsStore((state) => state.setImageAccountConcurrency);
  const setImageRedundantGenerationEnabled = useSettingsStore((state) => state.setImageRedundantGenerationEnabled);
  const setImageRedundantCopies = useSettingsStore((state) => state.setImageRedundantCopies);
  const setImageRedundantMaxAttempts = useSettingsStore((state) => state.setImageRedundantMaxAttempts);
  const setImageSettleEnabled = useSettingsStore((state) => state.setImageSettleEnabled);
  const setImageChatPanelEnabled = useSettingsStore((state) => state.setImageChatPanelEnabled);
      ...
      <CardContent className="space-y-4 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              value={String(config?.image_default_model_slug || "gpt-5-5-thinking")}
              onChange={(event) => setImageDefaultModelSlug(event.target.value)}
              placeholder="gpt-5-5-thinking"
              className="h-10 rounded-xl border-stone-200 bg-white"
            />
            <p className="text-xs text-stone-500">外部仍使用 gpt-image-2，底层 picture_v2 使用这里的模型 slug。</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-stone-700">生图兜底内部模型</label>
            <Input
              value={String(config?.image_fallback_model_slug || "gpt-5-3")}
              onChange={(event) => setImageFallbackModelSlug(event.target.value)}
              placeholder="gpt-5-3"
              className="h-10 rounded-xl border-stone-200 bg-white"
            />
            <p className="text-xs text-stone-500">保留给模型 slug 异常时的降级策略，建议先保持 gpt-5-3。</p>
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-700">
              <Checkbox
                checked={Boolean(config?.image_redundant_generation_enabled !== false)}
                onCheckedChange={(checked) => setImageRedundantGenerationEnabled(Boolean(checked))}
              />
              生图后台冗余生成
            </label>
          ...
          <div className="space-y-2">
            ...
            <Input
              value={String(config?.image_redundant_copies || "2")}
              onChange={(event) => setImageRedundantCopies(event.target.value)}
              placeholder="2"
          ...
          <div className="space-y-2">
            ...
            <Input
              value={String(config?.image_redundant_max_attempts || "3")}
              onChange={(event) => setImageRedundantMaxAttempts(event.target.value)}
              placeholder="3"
              className="h-10 rounded-xl border-stone-200 bg-white disabled:cursor-not-allowed disabled:opacity-50"
              disabled={config?.image_redundant_generation_enabled === false}
            />
            <p className="text-xs text-stone-500">同一轮全部失败才进入下一轮，最多 3 轮；有任意成功就按成功结果结束。</p>
          </div>
          <div className="space-y-2">
            <label className="flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-700">
              <Checkbox
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-700">
              <Checkbox
                checked={Boolean(config?.image_chat_panel_enabled !== false)}
                onCheckedChange={(checked) => setImageChatPanelEnabled(Boolean(checked))}
              />
              生图页显示文字聊天
            </label>
            <p className="text-xs text-stone-500">关闭后，文字聊天仍可通过顶部菜单进入独立页面。</p>
          </div>
          <div className="space-y-2">
